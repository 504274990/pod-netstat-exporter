...
func getPidForContainer(hostMountPath, id string) (int, error) {
	...
	attempts := []string{
		...
		// Adapt kubernetes 1.18+
		filepath.Join(hostMountPath, cgroupRoot, "..", "systemd", "kubepods.slice", "*", "kubepods*", "docker-"+id+".scope", "tasks"),
		// Adapt kubernetes use containerd
		filepath.Join(hostMountPath, cgroupRoot, "..", "pids", "system.slice", "containerd.service", "*"+id, "tasks"),
